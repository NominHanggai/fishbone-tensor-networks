...
class SpinBoson:
    ...
    def __init__(self, pd, coup_mat, freq, temp):
        ...
        self.iso_freq = np.concatenate((iso_freq,-iso_freq))
        print("coup_mat Zero Temp", [x[0,0] for x in coup_mat])
        iso_coup_mat = coup_mat[-1:]
        coup_mat = coup_mat[0:-1]
        coup_mat = np.concatenate((coup_mat, coup_mat))
        iso_coup_mat =np.concatenate((iso_coup_mat, iso_coup_mat))
        self.coup_mat = [mat * np.sqrt(np.abs(temp_factor(temp, self.freq[n])))
                for n, mat in enumerate(coup_mat)]
        self.iso_coup_mat = [mat * np.sqrt(np.abs(temp_factor(temp, self.iso_freq[n])))
                for n, mat in enumerate(iso_coup_mat)]
        print("self.iso_coup_mat", self.iso_coup_mat)
    ...
    def get_h2(self, t, delta, inc_sys=True):
        ...
        h2 = []
        for i, w in enumerate(self.iso_freq):
            d1 = self.pd_boson[::-1][i]
            d2 = self.pd_spin
            c = _c(d1)
            k = self.iso_coup_mat[i]

            h = kron((c+c.T), k) + kron(w*c.T@c, np.eye(d2))
            h = delta*h
            h2.append((h, d1, d2))

        for i, k in enumerate(d_nt_mat[0:self.len_boson-2*self.num_iso_modes]):
            d1 = self.pd_boson[::-1][i+2*self.num_iso_modes]
            d2 = self.pd_spin
            c1 = _c(d1)
            kc = k.conjugate()
    ...
    def get_u(self, t, dt, mode='normal', factor=1, inc_sys=True):
        ...
        for i, h_d1_d2 in enumerate(self.H):
            ...
            # u2 = np.transpose(u, [1,0,3,2]).reshape(s0*r0,s1*r1)
            # U1[i] = (r0, s0, u1)
            # U2[i] = (s0, r0, u2)
            u1 = u.reshape([r0, s0, r1, s1])
