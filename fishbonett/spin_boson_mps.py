...
class SpinBosonMPS:
    ...
    def __init__(self, pd_spin, pd_boson):
        def g_state(dim):
        ...
        self.pd = [pd_spin] + list(pd_boson)
        self.B = [g_state([1, d, 1]) for d in self.pd]
        self.S = [np.ones([1]) for d in self.pd]
        self.U = [np.eye(1) for d in pd_boson]
    ...
    def split_truncate_theta(self, theta, i: int, chi_max: int, eps: float, gpu=False):
        if gpu is False or CUPY_SUCCESS is False:
            ...
            chi_try = int(self.svd_expansion_factor * len(self.S[i + 1])) + 10
            ...
            while chivC == chi_try and chi_try < min(*theta.shape):
                print(f"Expanding chi_try by {self.svd_expansion_factor}")
                chi_try = int(round(self.svd_expansion_factor * chi_try))
                A, S, B = svd(theta, chi_try, full_matrices=False)
                chivC = min(chi_max, np.sum(S > eps), chi_try)
